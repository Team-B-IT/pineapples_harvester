...
class Plc():
	...
	def start(self):
		assert self.initPosition is None, \
...
class PlcListener(Thread):
	...
	def run(self):
		...
		while True:
			...
			if stateCode in [139, 140]:
				self.plc.state = __State
			if stateCode in [80]: # 80=cắt xong| 81,92=chọn mode nhưng bỏ qua
				self.plc.state = PlcState.AVAILABLE
			if stateCode in [81, 92]:
